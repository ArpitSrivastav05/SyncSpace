...
- **Rate limiting:** applied at the middleware layer using Upstash Redis as the counter store — both on AI endpoints (Technical Risks doc, Risk #3) and generally on auth-sensitive endpoints (login attempts, invite creation) to prevent abuse. **Phasing note (decided during Phase 5):** Redis isn't set up until Phase 7 (it's needed for the Socket.IO adapter regardless), so invite creation in Phase 5 uses a lightweight interim guard instead — a plain Prisma `count()` query in `workspaces.service.ts` capping invites per user per hour, no new infra dependency. This gets replaced by the proper Redis-backed limiter once Phase 7 stands Redis up anyway, at which point the interim check can simply be removed.
